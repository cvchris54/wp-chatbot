 ...
 *       welcomeMessage: 'Hi! Ask me anything about DraftSight products.',
 *       position:       'right'
 *     };
 *   </script>
...
(function () {
  ...
  var css = [
    ...
    '#' + WIDGET_ID + '_win{position:fixed;' + POS + ';bottom:86px;width:380px;height:560px;background:#fff;border-radius:16px;box-shadow:0 10px 40px rgba(0,0,0,.18);z-index:2147483645;display:flex;flex-direction:column;overflow:hidden;transition:opacity .22s,transform .22s}',
    ...
    '<div id="' + WIDGET_ID + '_win" class="cb_hidden" role="dialog" aria-label="' + TITLE + '">',
      '<div id="' + WIDGET_ID + '_hdr">',
        ...
        '<div style="flex:1;overflow:hidden">',
          ...
          '<div id="' + WIDGET_ID + '_hdr_sub">Online &bull; here to help</div>',
        '</div>',
    ...
    for (var i = 0; i < blocks.length; i++) {
      ...
      if (/^\d+\.\s/.test(lines[0])) {
        ...
        for (var j = 0; j < lines.length; j++) {
          var line = lines[j].replace(/^\d+\.\s+/, '');
          if (line) html += '<li>' + inlineMarkdown(line) + '</li>';
        }
        html += '</ol>';
      ...
      // Heading
        ...
        var heading = lines[0].replace(/^#{1,3}\s+/, '');
        html += '<h4>' + inlineMarkdown(heading) + '</h4>';
        if (lines.length > 1) {
          html += '<p>' + inlineMarkdown(lines.slice(1).join('<br>')) + '</p>';
        }

      // Paragraph (single lines within a block become <br>)
      } else {
        html += '<p>' + inlineMarkdown(lines.join('<br>')) + '</p>';
      }
    }
  ...
  function showTyping() {
    ...
    t.className = 'cb_typing';
    t.id = WIDGET_ID + '_typing';
