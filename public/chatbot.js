(function () {
  ...
  var css = '#' + WIDGET_ID + '_wrap *{box-sizing:border-box;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;margin:0;padding:0}'
    ...
    + '#' + WIDGET_ID + '_btn:hover{transform:scale(1.08)}'
    ...
    /* Window — 370px wide */
    + '#' + WIDGET_ID + '_win{position:fixed;' + POS + ';bottom:86px;width:370px;height:560px;background:#fff;border-radius:16px;box-shadow:0 10px 40px rgba(0,0,0,.18);z-index:2147483645;display:flex;flex-direction:column;overflow:hidden;transition:opacity .22s,transform .22s}'
    ...
    + '#' + WIDGET_ID + '_hdr_sub{color:rgba(255,255,255,.78);font-size:11px;margin-top:1px}'
    ...
    /* Messages container */
    + '#' + WIDGET_ID + '_msgs{flex:1;overflow-y:auto;padding:16px;display:flex;flex-direction:column;gap:10px;scroll-behavior:smooth}'
    + '#' + WIDGET_ID + '_msgs::-webkit-scrollbar{width:4px}'
    + '#' + WIDGET_ID + '_msgs::-webkit-scrollbar-thumb{background:#ddd;border-radius:2px}'

    /* Bubbles — max 78%, generous padding */
    + '.cb_msg{max-width:78%;border-radius:14px;font-size:14px;line-height:1.6;word-break:break-word;overflow-wrap:break-word;animation:cb_fadein .18s ease}'
    + '.cb_msg.bot{background:#f0f2f5;color:#1a1a1a;align-self:flex-start;border-bottom-left-radius:3px;padding:14px 16px}'
    + '.cb_msg.user{background:' + PRIMARY + ';color:#fff;align-self:flex-end;border-bottom-right-radius:3px;padding:11px 16px}'

    /* Markdown inside bot bubbles */
    + '.cb_msg.bot p{margin:0 0 8px}'
    + '.cb_msg.bot p:last-child{margin-bottom:0}'
    + '.cb_msg.bot strong{font-weight:600}'
    + '.cb_msg.bot em{font-style:italic}'

    /* Lists — numbers INSIDE the bubble, not hanging outside */
    + '.cb_msg.bot ol,.cb_msg.bot ul{margin:6px 0 8px;padding:0;list-style-position:inside}'
    + '.cb_msg.bot ol:last-child,.cb_msg.bot ul:last-child{margin-bottom:0}'
    + '.cb_msg.bot li{margin:5px 0;padding-left:4px;line-height:1.55}'
    + '.cb_msg.bot li strong{font-weight:600}'

    + '.cb_msg.bot h3,.cb_msg.bot h4{font-size:14px;font-weight:600;margin:8px 0 4px}'
    + '.cb_msg.bot a{color:' + PRIMARY + ';text-decoration:underline}'

    /* Typing dots */
    + '.cb_typing{display:flex;gap:5px;padding:14px 16px;background:#f0f2f5;border-radius:14px;border-bottom-left-radius:3px;align-self:flex-start}'
    + '.cb_typing span{width:8px;height:8px;background:#bbb;border-radius:50%;animation:cb_bounce 1.2s infinite}'
  ...
  wrap.innerHTML = '<button id="' + WIDGET_ID + '_btn" aria-label="Open chat">'
    ...
    + '<div style="flex:1;overflow:hidden"><div id="' + WIDGET_ID + '_hdr_title">' + TITLE + '</div>'
    + '<div id="' + WIDGET_ID + '_hdr_sub">Online &bull; here to help</div></div>'
    ...
    + '<button id="' + WIDGET_ID + '_send" disabled>'
